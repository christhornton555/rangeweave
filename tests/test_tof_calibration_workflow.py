"""Tests for the physical ToF calibration pose convention."""
...
class PivotPlanePoseTests(unittest.TestCase):
    def test_fronto_parallel_pose_is_z_equals_pivot_distance(self):
        pose = workflow.PivotPlanePose(800.0)
    ...
    def test_positive_rx_makes_bottom_farther_and_top_closer(self):
        pose = workflow.PivotPlanePose(800.0, rotation_x_deg=15.0)
    ...
    def test_positive_ry_makes_right_side_closer_and_left_side_farther(self):
        pose = workflow.PivotPlanePose(800.0, rotation_y_deg=15.0)
        geometry = pose.normal_and_offset()
        nx, ny, nz, offset = geometry
    ...
    def test_pose_can_be_attached_to_one_64_zone_observation(self):
        ...
        plane = workflow.calibration_plane_from_pivot_pose(
            pose,
            distances,
    ...
    def test_invalid_pose_is_rejected(self):
        for kwargs in (
            {"pivot_distance_mm": 0.0},
            {"pivot_distance_mm": -1.0},
            {"pivot_distance_mm": float("nan")},
            {"pivot_distance_mm": 800.0, "rotation_x_deg": float("inf")},
            {"pivot_distance_mm": 800.0, "rotation_x_deg": 100.0},
        ):
            with self.subTest(kwargs=kwargs):
                with self.assertRaises(calibration.CalibrationError):
                    workflow.PivotPlanePose(**kwargs)
